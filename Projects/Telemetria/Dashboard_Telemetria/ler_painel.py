...
def ler_eventos(img, y_rodape=None, debug_dir=None, tag=None, verbose=False):
    ...
    Nao depende de moldura nem de achar o titulo: varre a faixa abaixo dos
    manometros. Retorna {chave: (p, obs, conf)}."""
    H,W=img.shape[:2]
    y0=int(y_rodape) if y_rodape else int(0.80*H)
    y0=max(0,min(y0,H-20))
    x0=int(0.14*W)
    crop=img[y0:H, x0:W].copy()
    if crop.size==0: return {}
    # apaga textos em ciano (titulo "EVENTOS CRITICOS" e nomes de regiao),
    # que ficam na mesma linha dos itens em alguns layouts
    ...
    f=max(2.0,1400.0/max(crop.shape[1],1))
    ...
    if debug_dir:
        try: cv2.imwrite(os.path.join(debug_dir,"debug_EVENTOS_%s.png"%(tag or "crop")),th)
        except Exception: pass
    linhas=_ocr_linhas(th)
    out={}; vistos=[]
    for texto,conf,bbnum in linhas:
        ...
        if st: out.setdefault(k,(None,st,80)); continue
        val=float(m.group(1)+("."+m.group(2) if m.group(2) else "")) if m else None
        # releitura do numero pelo mesmo caminho dos manometros (votacao entre
        # binarizacoes e psm, com whitelist de digitos). Corrige casos como
        # 29,00 lido como 25,00 numa passada unica.
        if bbnum:
            bx,by,bw,bh=[int(v/f) for v in bbnum]
            mx=max(3,int(bw*0.12)); my=max(3,int(bh*0.30))
            sub=img[max(0,y0+by-my):min(H,y0+by+bh+my), max(0,x0+bx-mx):min(W,x0+bx+bw+mx)]
            if sub.size:
                v2,o2,c2=ler_valor(sub)
                if v2 is not None and (val is None or c2>=conf): val=v2; conf=max(conf,c2)
        if val is None: continue
        obs=None
        if not (lo<=val<=hi):
            obs="conferir (%s fora de %g a %g %s; leu %g)"%(k,lo,hi,u,val); val=None
        if k not in out or conf>out[k][2]: out[k]=(val,obs,conf)
    if verbose and not out:
        print("  [eventos] nada casou no rodape (y>=%d). Linhas lidas: %s"
              % (y0, "; ".join(v[:60] for v in vistos[:8]) or "(nenhuma)"))
    return out
