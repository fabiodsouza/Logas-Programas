...
const client = new Client({
  ...
  // FIXA a versao do WhatsApp Web a partir do CACHE LOCAL do projeto (arquivo
  // completo salvo em .wwebjs_cache por uma execucao real). Sem fixar, a lib
  // usa a versao ao vivo e, quando ela muda, a pagina recarrega no meio da
  // injecao e da o erro "Execution context was destroyed".
  // Antes isso apontava para um repositorio externo (wa-version), mas ele
  // APAGA versoes antigas de tempos em tempos - quando a versao fixada sumiu,
  // o download falhou e o erro voltou. Com o arquivo local, nao depende de
  // ninguem. Para atualizar no futuro: troque o numero abaixo por outro
  // arquivo .html que exista na pasta .wwebjs_cache.
  webVersion: '2.3000.1041788687',
  webVersionCache: {
    type: 'local',
...
client.on('ready', async () => {
  ...
  setInterval(async () => {
    try {
      ...
      if (est === 'CONNECTED') escreverHeartbeat();
    } catch (e) { /* nao atualiza: o vigia percebera e reiniciara */ }
  }, 60 * 1000);
});
