...
// Limite do log: ao passar disto, o arquivo atual vira .old (1 backup) e um
// novo comeca vazio. Evita que o log cresca sem parar e encha o disco (ENOSPC).
...
function _gravar(linha) {
  ...
  try {
    try {
      const st = fs.statSync(arquivoLog);
      if (st.size > LOG_MAX_BYTES) {
        try { fs.rmSync(arquivoLog + '.old', { force: true }); } catch (e) { /* ignora */ }
        fs.renameSync(arquivoLog, arquivoLog + '.old');
      }
    } catch (e) { /* arquivo ainda nao existe: segue */ }
    fs.appendFileSync(arquivoLog, prefixo + linha + '\r\n');
  } catch (e) { /* ignora */ }
}
