...
# Copias extras do dados.js. Serve para o envio no WhatsApp (resumo.js) ler o
# arquivo pela rede, na mesma pasta compartilhada das capturas.
# Para mudar sem mexer no codigo: defina a variavel de ambiente
#   LOGAS_COPIAS_DADOS=\\\\WINDOWS\\capturas\\dados.js;D:\\backup\\dados.js
COPIAS_DADOS_JS = [
    r"\\WINDOWS\capturas\dados.js",
]
if os.environ.get("LOGAS_COPIAS_DADOS"):
    COPIAS_DADOS_JS = [c.strip() for c in os.environ["LOGAS_COPIAS_DADOS"].split(";") if c.strip()]

def _gravar_atomico(texto, destino):
    """Grava em arquivo temporario NA MESMA PASTA e so entao renomeia.

    Quem le pela rede (o resumo do WhatsApp) nunca pega o arquivo pela metade:
    ou ve a versao anterior inteira, ou a nova inteira."""
    pasta = os.path.dirname(os.path.abspath(destino))
    if not os.path.isdir(pasta):
        # Em caminho de rede, NAO criamos a pasta: se ela nao existe, o mais
        # provavel e erro de digitacao ou compartilhamento fora do ar - criar
        # uma pasta nova so esconderia o problema.
        if destino.startswith("\\\\") or destino.startswith("//"):
            raise FileNotFoundError("pasta de rede indisponivel: %s" % pasta)
        os.makedirs(pasta, exist_ok=True)
    tmp = os.path.join(pasta, ".dados.js.tmp")
    with open(tmp, "w", encoding="utf-8") as f:
        f.write(texto)
        f.flush()
        os.fsync(f.fileno())
    os.replace(tmp, destino)   # troca instantanea, mesmo com leitor aberto

def copiar_dados_js(texto, destinos=None):
    """Replica o dados.js nas pastas extras. Falha de rede NAO derruba o OCR."""
    for d in (destinos if destinos is not None else COPIAS_DADOS_JS):
        try:
            _gravar_atomico(texto, d)
            print("copia do dados.js:", d)
        except Exception as e:
            print("  [AVISO] nao consegui copiar o dados.js para %s: %s" % (d, e))

def escrever_dados_js(data,path=DADOS_JS,copias=True):
    ...
    L+=["  }","};",""]
    texto="\n".join(L)
    _gravar_atomico(texto, path)
    if copias: copiar_dados_js(texto)
