...
def ler_dados() -> dict:
    ...
    try:
        ...
        return {"atualizado": None, "leituras": {}}
...
def consumo_medido(ls: list[dict], cfg: dict) -> float | None:
...
def api_excel(request: Request, horas: int = 168):
    ...
    for p in dados["pontos"]:
        ...
        lin = [
            ...
            h, dt_local(cheg), dt_local(p.get("medido_em")), ROT.get(st, ""),
        ]
        if conj:
    ...
    cabecalho = ["Ponto", "Pressão (Bar)", "Reserva mínima (Bar)",
                 ...
                 "Autonomia (h)", "Reabastecimento previsto", "Medido em", "Situação"]
    larguras = [22, 13, 18, 16, 12, 22, 18, 18]
    formatos = [None, "0.00", "0.00", "0.00", "0.0",
                "dd/mm/yyyy hh:mm", "dd/mm/yyyy hh:mm", None]
    if conj:
