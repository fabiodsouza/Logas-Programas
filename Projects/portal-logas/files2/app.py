...
def pontos_disponiveis() -> list:
    """Pontos do dados.js, sem as leituras cujo nome o OCR não identificou."""
    return sorted(k for k in ler_dados().get("leituras", {}) if not k.startswith("?@"))
...
def estado_ponto(dados: dict, cfg: dict, ponto: str, horas_hist: int) -> dict:
    ...
    if len(val) >= 2 and val[-1]["t"] == reg["t"]:
        ant = val[-2]
...
def entrar(request: Request, login: str = Form(...), senha: str = Form(...)):
    ...
    chave = f"{request.client.host if request.client else '?'}|{login}"
    if bloqueado(chave):
        return JSONResponse(
...
def admin_login(request: Request, login: str = Form(...), senha: str = Form(...)):
    login = login.strip().lower()
    chave = f"admin|{request.client.host if request.client else '?'}|{login}"
    if bloqueado(chave):
        return JSONResponse(
    ...
    if not senha_confere(login, senha, papel="admin"):
        registrar_falha(chave)
        registrar_log(login, "tentativa de login administrativo falhou")
        return JSONResponse({"erro": "Login ou senha incorretos."}, status_code=401)

    registrar_log(login, "entrou no painel administrativo")
...
@app.get("/admin/api/panorama")
def admin_panorama(request: Request):
    ...
    for login, reg in clientes().items():
        for p in reg.get("pontos", []):
            dono[p] = reg.get("nome", login)

    linhas = []
    for ponto in pontos_disponiveis():
        e = estado_ponto(dados, cfg, ponto, 24)
        linhas.append({
            "ponto": ponto,
            "cliente": dono.get(ponto),
            "pressao": e.get("pressao"),
            "minimo": e.get("minimo"),
            "status": e.get("status"),
            "consumo_barh": e.get("consumo_barh"),
            "media_dia_bar": e.get("media_dia_bar"),
            "horas_ate_limite": e.get("horas_ate_limite"),
            "limite_em": e.get("limite_em"),
            "chegada_prevista": e.get("chegada_prevista"),
            "medido_em": e.get("medido_em"),
            "sem_sinal": e.get("sem_sinal"),
            "semana": e.get("semana"),
            "trocas": {k: v for k, v in (e.get("trocas") or {}).items() if k != "eventos"},
        })

    ordem = {"abaixo": 0, "critico": 1, "atencao": 2, "sem_sinal": 3, "ok": 4, "sem_dados": 5}
    linhas.sort(key=lambda x: (ordem.get(x["status"], 9), x["horas_ate_limite"] is None,
                               x["horas_ate_limite"] or 0))
    return JSONResponse({"atualizado": dados.get("atualizado"), "pontos": linhas})
