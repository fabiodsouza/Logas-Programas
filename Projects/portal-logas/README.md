...
### Caminho 2: IIS como proxy reverso

Serve quando você não quer um processo do Cloudflare rodando no servidor. O
portal continua escutando só em `127.0.0.1:8100`; o IIS é quem atende a
internet e repassa.

1. **Módulos.** Instale o URL Rewrite e o Application Request Routing:
   https://www.iis.net/downloads/microsoft/url-rewrite e
   https://www.iis.net/downloads/microsoft/application-request-routing
2. **Ligue o proxy.** No IIS Manager, clique no nó do servidor (não no site),
   abra "Application Request Routing Cache", e em Server Proxy Settings marque
   "Enable proxy".
3. **Libere as variáveis.** Ainda no nó do servidor, abra "URL Rewrite", clique
   em "View Server Variables" e adicione `HTTP_X_FORWARDED_FOR` e
   `HTTP_X_FORWARDED_PROTO`. Sem isso o `web.config` é recusado.
4. **Crie o site** apontando para uma pasta vazia (ex.: `C:\inetpub\portal-logas`),
   com host name `consumo.logas.com.br`, e copie para lá o `web.config.exemplo`
   deste pacote, renomeando para `web.config`.
5. **Certificado.** Duas opções, as duas gratuitas:
   - **win-acme** (https://www.win-acme.com): gera certificado Let's Encrypt,
     faz a ligação no IIS e cria a tarefa de renovação automática. Precisa da
     porta 80 acessível de fora durante a validação.
   - **Certificado de origem da Cloudflare**: vale 15 anos e nunca renova, mas
     só funciona com o proxy da Cloudflare ligado (nuvem laranja) e o modo
     SSL/TLS em "Full (strict)". Vem em PEM e precisa ser convertido para PFX
     antes de importar no IIS.
6. **Portal.** Ponha `"confiar_proxy": true` no `config.json` e reinicie o
   serviço, para o portal passar a ler o IP real do visitante.
7. **Firewall.** Libere a 443 (e a 80, se usar win-acme). Se o tráfego passa
   pela Cloudflare, vale restringir a entrada apenas às faixas de IP dela
   (https://www.cloudflare.com/ips/): aí ninguém alcança o servidor direto.

O `/admin` fica acessível no mesmo endereço. Para deixá-lo só interno, acrescente
uma regra no `web.config` bloqueando `/admin` e acesse pelo próprio servidor em
...
## Fuso, CDN, codificação e leitura do dados.js

Quatro decisões que valem saber, porque foram problemas que apareceram no teste:

- Os scripts `.ps1` estão em ASCII puro e com BOM de UTF-8. O PowerShell 5.1 lê
  `.ps1` como ANSI, então um acento ou um travessão no meio de um texto quebra o
  interpretador. Se você editar esses arquivos, não use acento.
- O portal lê JSON com `utf-8-sig`, porque o `Set-Content -Encoding UTF8` do
  PowerShell grava BOM e o `json.loads` engasga nele. Vale para o `config.json`,
  o `clientes.json`, o `admins.json` e o próprio `dados.js`.
...
Cada ponto aparece como um bloco com tiles, no mesmo formato do detalhe da Torre
de Controle: leitura anterior, pressão atual, consumo na última hora, consumo
médio, reserva mínima, autonomia, quando chega na reserva, reabastecimento
previsto e última troca de carreta. Ficam de fora os tiles internos da Torre
(base de abastecimento, tempo de viagem, "sair da base até").

- **Pressão atual** em Bar, com a hora da foto. Se o ponto está sem sinal, o
  portal projeta a queda pelo consumo médio e diz que é projeção.
- **Consumo na última hora**: a diferença real entre as duas últimas fotos. Se
  entre elas houve troca de carreta, o tile diz isso em vez de mostrar consumo
  negativo.
- **Consumo médio** em Bar/h, média das últimas quatro quedas reais. Subida de
  25 Bar ou mais entre duas fotos é lida como troca de carreta e não entra na
  conta de consumo (ajuste esse limite em `salto_troca`, se necessário). Queda acima de 100 Bar/h
  e valor acima de 260 Bar são descartados como erro de leitura, igual ao painel.
- **Autonomia** até a reserva mínima do ponto (30 Bar, salvo ajuste no
  `config.json`), com a data e hora em que a reserva será alcançada.
...
- **Consumo por dia**, em barras, 2 semanas ou 1 mês. Soma as quedas reais de
  cada dia e ignora os saltos de troca de carreta. O dia em andamento aparece em
  tom mais claro e não entra na média. Ao lado, média por dia, total dos últimos
  7 dias e comparação com os 7 dias anteriores, em porcentagem.
- **Reabastecimentos**: data e hora de cada troca de carreta, pressão antes e
  depois, quantas houve na semana e o intervalo médio entre elas. As trocas
  também aparecem marcadas com um triângulo no gráfico de pressão.
- **Excel** com cinco abas — Resumo, Leituras, Consumo por dia, Trocas de
  carreta e Informações — do período escolhido na tela, em Arial, com filtro e
  cabeçalho fixo.

### Mostrar em m³ em vez de Bar

Cliente pensa em metro cúbico, não em Bar. Preencha `m3_por_bar` no
`config.json` com quantos m³ correspondem a 1 Bar de queda naquele ponto, e o
portal passa a exibir consumo, média diária e gráfico em m³, mantendo a pressão
em Bar:

```json
"m3_por_bar": { "ORLÂNDIA": 11.4, "FORTLEV": 11.4 }
```

O jeito mais confiável de achar esse número é pela sua própria nota: pegue os m³
faturados de uma carreta e divida pelos Bar consumidos daquela carreta, do cheio
até a troca. Se a frota é padronizada, um valor serve para todos os pontos.
Enquanto a chave ficar vazia, tudo continua em Bar.
